...
# =============================================================================
# THE ONE HOLE IN THE GATE — an unmade DRINK Helen has rewritten, #1137
# =============================================================================
# Both flags are parameterised here, where the fixtures above pin all but one:
# the rule under test is about the COMBINATION, so a fixture that can only vary
# `proofread` cannot express it.
...
def test_a_rewritten_drink_publishes_with_a_ticket_open_and_food_still_does_not():
    """`awaiting_fix: true` stops holding a DRINK back once Helen has rewritten
    it. GitHub issue #1137, her ruling 2026-09-17.

    WHY THE HOLE EXISTS, in her words: "One of the uses of this site for me is
    to test recipes, and it's been annoying exporting PDFs from the food site so
    I can do that from my iPad. The stakes are much lower for cocktails, and
    they're much easier for me (and you) to rewrite too." A drink she is about
    to go and MAKE is the one she most needs to read on a screen in the kitchen,
    and `awaiting_fix: true` -- "I've proofread, but one small thing has been
    raised as a ticket" (2026-09-01) -- is exactly what such a drink carries.

    FOUR FIXTURES, ONE BUILD, BECAUSE THE RULE IS A TRUTH TABLE and any one of
    them alone is satisfied by a plugin that does nothing, or by one that drops
    everything. Each differs from the publishable case in exactly one key:

        drink, rewritten,     ticket open, proofread  -> PUBLISHES (the feature)
        drink, NOT rewritten, ticket open, proofread  -> held  (needs her claim)
        drink, rewritten,     ticket open, unproofread-> held  (never forgiven)
        FOOD,  rewritten,     ticket open, proofread  -> held  (cocktails only)

    The last row is the one that matters most and is the easiest to lose: the
    exemption is scoped to the `cocktail_recipes` collection, and a future
    refactor that reads the flags without the collection name would quietly put
    every ticketed food recipe on the live site.
    """
    _require_bundler()
    out = ROOT / "tmp" / "_test_site_1137"
    recipes = ROOT / "_cocktail_recipes"
    created_dir = not recipes.exists()
    made = []
    try:
        recipes.mkdir(exist_ok=True)
        drinks = {
            # slug: (rewritten, awaiting_fix, proofread, should_publish)
            "zzz-1137-drink-rewritten":     ("true",  "true", "true",  True),
            "zzz-1137-drink-not-rewritten": ("false", "true", "true",  False),
            "zzz-1137-drink-unproofread":   ("true",  "true", "false", False),
        }
        for slug, (rw, af, pr, _) in drinks.items():
            p = recipes / f"{slug}.md"
            p.write_text(
                GATE_DRINK_ANY.format(t=slug, rewritten=rw, awaiting_fix=af,
                                      proofread=pr),
                encoding="utf-8")
            made.append(p)

        food = ROOT / "_food_recipes" / "zzz-1137-food-rewritten.md"
        food.write_text(
            GATE_FOOD_ANY.format(t="zzz-1137-food-rewritten", rewritten="true",
                                 awaiting_fix="true", proofread="true"),
            encoding="utf-8")
        made.append(food)
        ...
        assert drink_live("zzz-1137-drink-rewritten"), (
            "A drink with `rewritten: true`, `proofread: true` and a ticket "
            "open (`awaiting_fix: true`) was HELD BACK. #1137 is the whole "
            "point of that combination: Helen reads the site while making a "
            "drink she has not made before. _plugins/publish_gate.rb must "
            "forgive the `awaiting_fix` leg for `cocktail_recipes` when "
            "`rewritten == true`."
        )
        assert not drink_live("zzz-1137-drink-not-rewritten"), (
            "A drink with a ticket open and `rewritten: false` was PUBLISHED. "
            "The exemption is Helen's own claim to have rewritten it "
            "(MANUAL 9.1.1: `rewritten: true` is never an agent's keystroke), "
            "so without that claim the gate's first leg still applies."
        )
        assert not drink_live("zzz-1137-drink-unproofread"), (
            "A drink with `proofread: false` was PUBLISHED. #1137 forgives the "
            "TICKET, never the proofread -- that is 'the very last touch that "
            "I, the human, make to the file' (#667), and #1137 does not touch "
            "it. Everything Helen has not read is now live."
        )
        assert not (out / "food" / "recipes" / "zzz-1137-food-rewritten"
                    / "index.html").exists(), (
            "A FOOD recipe with a ticket open was PUBLISHED because it says "
            "`rewritten: true`. #1137 is cocktails-only and Helen said why: "
            "'The stakes are much lower for cocktails, and they're much easier "
            "for me (and you) to rewrite too.' The exemption must be scoped to "
            "the `cocktail_recipes` collection, not to the flags alone."
        )
    finally:
        for p in made:
            p.unlink(missing_ok=True)
