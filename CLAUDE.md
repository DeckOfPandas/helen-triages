...
- **`meta.awaiting_fix` FAILS CLOSED: a recipe publishes only if it says `awaiting_fix: false`.** Not "true hides it" -- false is the only value that lets a page through. Missing flag, old hyphenated `awaiting-fix`, or a quoted string: all held back, and all break a test, which stops the build (#369). Never write the hyphenated form: Liquid parses `page.meta.awaiting-fix` as SUBTRACTION, so a template asking whether a page is flagged gets arithmetic and publishes it.
