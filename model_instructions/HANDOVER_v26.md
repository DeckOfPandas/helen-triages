...
>
> **One thing to know before touching `cooking_methods.yml`:** `groups` and
> `groups[].columns` are now read by nothing at all. The deleted include was
> their only consumer; the calculator reads `methods` and their own fields.
> They are left in place deliberately — the file is generated by two scripts
> pinned to commit `8bdbd27`, so hand-editing it would make it disagree with
> its own generator — and `test_group_columns_are_all_renderable` was retired
> rather than left passing over a template that no longer exists. See the note
> where it stood in `tests/test_reference_data.py`, and issue #400.
