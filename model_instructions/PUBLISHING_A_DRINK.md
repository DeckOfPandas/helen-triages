...
**THERE ARE TWO STAGING FOLDERS FOR COCKTAILS SINCE 2026-09-18**: `4-promote/`,
...
For a batch in progress there is **one** working copy of the private cocktails
...
1. **Helen moves** a cocktail recipe into `_cocktail_drafts/4-promote/`. The move is the
   ...
   - **She sets `rewritten: true` herself — #1137, 2026-09-17.** The move used
     ...
     LEG OF THE PUBLISH GATE for cocktails now, and a flag that decides whether a
     page exists belongs with `proofread` — hers to type, never an agent's.
     **If a cocktail recipe in here still says `rewritten: false`, it will not publish:
...
5. **Helen proofreads** by reading each cocktail recipe's built page on her own dev
   ...
   - **SAY WHICH REPO THE BRANCH IS IN, AND HOW TO GET IT. THE URLs ARE
     ...
     **The general rule: "pushed" is not "she can see it".** Every handover in
     this file crosses two repos, and the cocktails half is always the one that
     needs the extra step.
   - If a small thing is wrong: `awaiting_fix: true` in a commit that says
     what; fix between them; Helen re-reads (the whole page — cocktail recipes
     are short); flag back.
...
6. **Helen promotes**, herself, always, unless she explicitly asks Claude to:
   ...
   **SIX THINGS THAT PROMOTION TURNED OUT TO NEED, none of them obvious
   ...
   - **CHECK THE PROMOTED PAGES BY NAME IN THE PRODUCTION BUILD.** `sh
     ...
     so a cocktail recipe with a flag wrong is simply ABSENT — which looks exactly like
...
- `rewritten: true` — the words are Helen's. **Only she writes it, and since
  ...
  It is the cocktails gate's third leg, so a cocktail recipe with it `false` does
  not publish at all.
- `made_before: false` — she has not made it yet, and **it publishes anyway**
  ...
  she reads the cocktail recipe there while making it, which beats exporting a PDF. The
  pair is the rule and the two halves are easy to swap by mistake: **unmade is
  fine, unrewritten is not.** She rewrites a cocktail recipe before she makes it.
...
- `proofread: true` — she read the rendered page. Any agent edit after that
  sets it false again in the same commit (issue #367), which takes the cocktail
  recipe off the live site until she reads it again. That is the point.

## What "promotion-ready" means for the data

In `4-promote/`, in `5-final-proofread/` and in `_cocktail_recipes/` a cocktail
recipe is in its published tense: every `suggestion` is a bottle's canonical name
...
`test_item_is_gone_once_the_generic_is_filled_in` — so by the time a cocktail
recipe is promotable the field went long ago. A fresh ingest still writes it, beside
